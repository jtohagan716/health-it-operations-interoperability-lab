...
the HL7 v2 coding system represents LOINC,
the LOINC code is supported by the lab contract,
the display matches the expected clinical concept,
the unit is allowed for that supported concept.
...
The terminology-validation pattern established here is:
